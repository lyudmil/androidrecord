...
public abstract class ActiveRecordBase<T extends ActiveRecordBase> {
    ...
    public T find(String whereClause) {
        return new QueryContext<T>(database, (Class<T>) getClass()).find(whereClause);
    }
    ...
    public List<T> where(String whereClause) {
        return new QueryContext<T>(database, (Class<T>) getClass()).where(whereClause);
    }
     ...
     */
    public List<T> all() {
        return new QueryContext<T>(database, (Class<T>) getClass()).all();
    }
     ...
     *
     * The table name can be overridden using the {@link Table} annotation and specifying a name attribute.
     *
     * @param activeRecordClass the persistent class to generate a table name for.
     *
    ...
    public static String tableNameFor(Class activeRecordClass) {
        if (activeRecordClass.isAnnotationPresent(Table.class)) {
            Table annotation = (Table) activeRecordClass.getAnnotation(Table.class);
